...
size_t Database::RemoveIf(function<bool(const Date &date, const string &event)> pred) {
    ...
    for (auto &i : _base) {
        ...
        for (auto it1 = it; it1 != events.end(); ++it1) _baseSet[date].erase(*it1);
        events.erase(it, events.end());
        if (events.empty()) emptyKeys.push_back(date);
    }
    for (const auto &i : emptyKeys) _base.erase(i);
...
BaseEntry Database::Last(const Date &date) const {
    ...
    if (it == _base.begin()) throw invalid_argument("Date is too low");
    --it;
